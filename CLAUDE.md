...
Use this python if needed:

- For linux: \$HOME/miniforge3/envs/SeZM/bin/python
- For mac: \$HOME/Software/miniforge3/envs/dpmd/bin/python
...
1. **Multi-backend Unified Interface**:
...
1. Inherit from `BaseDescriptor` and implement necessary methods
1. Add corresponding implementations in each backend (tf/pt/jax/pd)
1. Add unit tests
...
In interval [x_i, x\_\{i+1}], for variable x, the polynomial is:
...
- 修改完代码后，需要更新 ./doc/outisli/dpa4.md，不需要体现版本迭代或者修改了哪些内容，直接完善即可。
- 我最讨厌ai agent怕麻烦，就偷懒，你应该首先plan好，然后一步一步认真细致执行，无限预算
- 不需要保持兼容性代码，现在这个模型处于初始开发阶段，version永远是1，不需要兼容老代码
- 如果有参数修改，记得修改 deepmd/utils/argcheck.py 以及 examples/water/dpa4/input.json

Python 环境：

- H20上: /aisi-vepfs/outisli/miniforge3/envs/dpmd/bin/python
- Mac上: \$HOME/Software/miniforge3/envs/dpmd/bin/python
- （如果没找到那可能是叫dpmd，或者其他conda/mamba路径）
- Use this command to test: OMP_NUM_THREADS=1 DP_INTER_OP_PARALLELISM_THREADS=0 DP_INTRA_OP_PARALLELISM_THREADS=0 XXX/envs/XXX/bin/python -m pytest source/tests/pt/model/test_descriptor_sezm.py source/tests/pt/model/test_descriptor_sezm_triton.py source/tests/pt/model/test_sezm_model.py -v 2>&1 | head -200

如果没有修改model，就不需要运行model的test，因为会compile，时间太长。
