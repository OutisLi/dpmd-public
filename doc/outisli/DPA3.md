...
1. **后端选择**: 根据 `backend` 参数选择 PyTorch 后端
1. **训练函数调用**: 调用 `deepmd.pt.entrypoints.main.train()`
...
1. **DPA3 初始化**: 当 descriptor 类型为 `"dpa3"` 时，创建 `DescrptDPA3` 实例
1. **模型组装**: 将描述符与拟合网络组合成完整模型
...
1. **全局精度转换**: 将输入数据转换为 `GLOBAL_PT_FLOAT_PRECISION`
1. **模型计算**: 使用模型组件指定的精度进行计算
1. **输出类型转换**: `output_type_cast()` 将输出转换回输入精度
...
1. **性能**: 降低精度通常可以提高计算速度和减少内存使用
1. **数值稳定性**: 高精度有助于数值稳定性，特别是在训练初期
1. **能量精度**: 能量相关计算始终使用 `GLOBAL_ENER_FLOAT_PRECISION`，通常为 float64，因此模型在推理输出到时候默认还是双精度（即 lammps 调用时）
...
1. **批处理合并**: `collate_batch()` 组合多个帧
1. **设备转移**: 数据移动到 GPU/CPU
1. **输入分离**: 模型输入与标签分离
...
1. **预计算存储**: 预先计算并存储常用距离范围内的描述符值
1. **插值查表**: 推理时通过插值查表替代复杂的神经网络计算
1. **内存换时间**: 牺牲一定内存空间换取显著的计算速度提升
...
1. **参数调优**: 根据具体应用场景调整步长参数
1. **内存监控**: 关注压缩后的内存使用情况
1. **性能测试**: 定量测试压缩带来的性能提升效果
...
1. **训练集成**: 在 `Trainer` 类中被包装用于训练过程
1. **推理支持**: 在 `DeepEval` 类中用于模型推理和部署
1. **多任务支持**: 支持多种物理性质的联合训练和预测
...
1. **边更新**: 基于节点和角度信息更新边表示
1. **角度更新**: 基于节点和边信息更新角度表示
1. **残差连接**: 支持多种残差连接策略
...
1. **系统能量聚合**: 在 `fit_output_to_model_output` 中将原子能量聚合成系统能量
1. **自动微分支持**: 力的计算通过自动微分实现，保持梯度传递
...
1. **类型嵌入**: 计算原子类型嵌入向量
1. **RepFlow 计算**: 通过多层 RepFlow 处理节点、边和角度信息
1. **输出生成**: 生成最终的原子环境描述符
...
1. **梯度禁用**: 推理时自动禁用梯度计算
1. **批处理优化**: 通过自动批处理控制内存使用
1. **设备内存管理**: 自动管理 GPU 内存分配和释放
