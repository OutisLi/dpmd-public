...
def _forward_fake(
    ...
    descriptor = edge_vec.new_empty(atype.shape[0], 6 * channels)
    state = edge_vec.new_empty(atype.shape[0], 19, channels)
...
def dpa4c_canonical_compress_energy_force(
    ...
    with disable_proxy_modes_tracing():
        stride, table_max, rcut, eps, degree_floor = (
            float(value) for value in descriptor.compress_info.tolist()
        )
    operator_args = (
        graph.source,
    ...
    node_descriptor, state = torch.ops.deepmd.dpa4c_canonical_compress(
        graph.edge_vec,
        *operator_args,
    )
    ...
    *hidden, head = fitting.nets[0].layers
    ...
    edge_gradient = torch.ops.deepmd.dpa4c_canonical_compress_backward(
        descriptor_gradient,
        state,
        graph.edge_vec,
        *operator_args,
